...
<details>
<summary>📅 2026-05-15: Analysisタブ棒グラフの表示バグ調査・修正</summary>

### 🛠 実装内容
- Analysisタブの棒グラフに要素が6〜7件しか表示されない原因を調査するデバッグコードを実装（`4_Analysis.py` に `st.expander`、`analysis.py` に `print([DEBUG]...)` を追加）
- 原因特定後、以下の3件のバグを修正
  1. **タグ種類が少ない（仮説3確定）**：`_TITLE_TAG_MAP` を15種→45種に拡充、`_seed_tags_by_difficulty` を各難易度帯2種→5種に拡充、問題あたりのタグ上限を `[:3]` → `[:5]` に変更
  2. **タグの強制上書き漏れ**：`sync_tags` が `tags == None` の問題のみ対象だったため、既存の旧タグが更新されなかった。`all()` で全問題を取得して強制上書きするよう修正
  3. **提出ページネーション未実装**：`fetch_user_submissions` が1回のAPIコールのみで最大500件打ち切りだった。`while True` ループで `from_second` をずらしながら0件になるまで全ページ取得するよう修正
- **負の難易度（A/B問題）のグラフ除外バグ修正**：`difficulty < 0` で `continue` していた処理を削除し、`max(diff, 0)` で強制的に灰バケットへカウントするよう修正
- 調査用デバッグコードをすべて削除してクリーンアップ

### ⚠️ 発生したエラーと対策
- タグ同期後もタグ種類が7件のまま変わらなかった → `untagged` のみを対象にしていたため、既存タグを持つ問題がスキップされていた。`all()` に変更して解決
- 提出同期で新規登録が0件 → 問題マスタ未同期、または500件打ち切りにより以降の提出がDBに入っていなかった。ページネーション修正と問題マスタ再同期で解決
- `uvicorn main:app --reload` でモジュール不明エラー → `backend/app/main.py` は `from app.xxx` でインポートしているため、`backend/` ディレクトリから `uvicorn app.main:app --reload` で起動する必要がある

### 💡 技術的ポイント
- AtCoder Problems API の提出一覧エンドポイントは1リクエスト最大500件・`epoch_second` 昇順で返す。全件取得には「0件が返るまで `from_second` をずらしてループ」するページネーションが必須
- `< _PAGE_SIZE` での早期終了はエッジケース（最終ページがちょうど500件）で誤動作するため、終了条件は「0件」のみにするのが堅牢
- `sync_tags` のような「全件更新」ユースケースでは `filter(tags == None)` で未設定のみを対象にするのではなく、`all()` で全件取得して強制上書きすることが重要
